...
def find_polynomial_regression(deg, data):
    coef = np.polyfit(np.arange(len(data)), data, deg)
    p = np.poly1d(coef)
    return p
...
def fill_data_object(dates, cases, p):    
    ...
    pred_dates = copy.deepcopy(dates)
...
if __name__ == "__main__":    
    ...
    deg = 9
    p = find_polynomial_regression(deg, df["cases"].to_list()) 
    
    dates = df["dates"].dt.strftime("%d.%m.%Y").astype(str).to_list() # Convert date format 
    
    data = fill_data_object(dates, df["cases"].to_list(), p)
